...
void MEMSInterface::onDataReceived(const MEMSData &data)
{
    ...
    const QList<QAbstractAxis *> horizontalAxes = m_chart->axes(Qt::Horizontal);
    if (!horizontalAxes.isEmpty()) {
        if (QValueAxis *axisX = qobject_cast<QValueAxis *>(horizontalAxes.first())) {
            axisX->setRange(static_cast<qreal>(now - 60000), static_cast<qreal>(now));
        }
    }
